...
// dev server — the whole file runs serially (not just each describe
// block) so no two tests here ever race on it. Other spec files never
// touch the store, so they're unaffected either way.
test.describe.configure({ mode: "serial" });

const STORE_PATH = path.join(process.cwd(), ".data", "scores.json");

function writeStore(games: Game[], updatedAt: string | null = new Date().toISOString()) {
  fs.mkdirSync(path.dirname(STORE_PATH), { recursive: true });
  fs.writeFileSync(STORE_PATH, JSON.stringify({ games, updatedAt, source: "balldontlie" }));
}
...
test.describe("cron route", () => {
  ...
  test("two concurrent authenticated calls leave the store as valid JSON", async ({ request }) => {
    ...
    // The two calls above may have written a valid store (a real
    // SPORTS_API_KEY) or left an empty/missing one untouched (no key
    // configured) — either way, readScores()'s contract is that the file,
    // if present, is always valid JSON, never a half-written interleave.
    if (fs.existsSync(STORE_PATH)) {
      expect(() => JSON.parse(fs.readFileSync(STORE_PATH, "utf-8"))).not.toThrow();
    }
  });
});
